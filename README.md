...
- **Multiprocess Prometheus registry** — the API runs with `--workers 2` and the worker runs with `--concurrency 4`; `prometheus_client`'s default registry is per-process and doesn't aggregate across any of that. `PROMETHEUS_MULTIPROC_DIR` makes every process write to a shared directory (a Docker volume in compose, an `emptyDir` in k8s), and the API's `/metrics` endpoint uses `MultiProcessCollector` to merge them into one scrape — this is also how worker-side counters (job duration, retries, DLQ size) reach Prometheus at all, since the worker has no HTTP surface of its own. The per-worker gauges (`active_workers`, `dlq_size`) use `multiprocess_mode="livesum"` so live processes' values combine into one total, and each worker calls `mark_process_dead` on exit so a restarted worker doesn't leave a stale count behind. `queue_depth` is not tracked with a counter at all: an inc/dec gauge drifts once jobs retry, so the `/metrics` endpoint reads the queue lengths straight from the broker (`LLEN` on the Redis lists) at scrape time, which is the broker's own source of truth. The API deliberately does not wipe the shared directory on startup (that would erase the workers' live metrics); a clean slate comes from recreating the volume with `docker compose down -v`, and the worker's liveness probe runs with `PROMETHEUS_MULTIPROC_DIR` unset so short-lived probe processes never leave stray metric files behind.
